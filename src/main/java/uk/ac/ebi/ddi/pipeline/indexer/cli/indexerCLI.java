...
public class indexerCLI {
    ...
    public static void main(String[] args) throws Exception {
        ...
        options.addOption(reportFileOpt, true,
                "Record errors/warn messages into outfile. If not set, print message on the screen.");

        String source  = "source";
        options.addOption(source, true,
                "input the source from the config file, the current sources are: "
                        + ConfigurationFileBootstrap.getSourceNames(config).toString());
        ...
        options.addOption(commandLine, true,
                "This command specify the process that will take in the backend from the possible command: "
                        + CommandOptions.getValuesName().toString());
