    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        if(datasetSimilars != null && !datasetSimilars.isEmpty()){
            for(int i = 0; i < datasetSimilars.size(); i++){
                DatasetStatInfo dataset = datasetSimilars.get(i);
                ...
                if(dataset.getIntersectionInfos() != null){
                    Collections.sort(dataset.getIntersectionInfos(), (IntersectionInfo inter1, IntersectionInfo inter2) -> Double.compare(inter2.getCosineScore(), inter1.getCosineScore()));
